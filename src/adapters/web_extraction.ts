...
async function findYtDlp(): Promise<string> {
    // Try to find yt-dlp in winget packages first (Windows)
    const wingetPath = path.join(process.env.LOCALAPPDATA || '', 'Microsoft', 'WinGet', 'Packages');
    
    if (wingetPath) {
        try {
            const { stdout } = await execAsync(`powershell "Get-ChildItem -Path '${wingetPath}' -Recurse -Filter 'yt-dlp.exe' | Select-Object -First 1 -ExpandProperty FullName"`);
 ...
 */
function parseJSON3(json3Data: JSON3Format, language: string): TranscriptSegment[] {
    ...
    for (const event of json3Data.events) {
        ...
        for (let i = 0; i < event.segs.length; i++) {
            ...
            currentTimeMs = event.tStartMs + (nextSeg?.tOffsetMs || event.dDurationMs);
...
/**
 * Fetches a transcript using yt-dlp (web extraction).
 */
export async function getYouTubeTranscript(videoUrl: string, language: string = 'en'): Promise<TranscriptSegment[]> {
    ...
    try {
        ...
        const segments = parseJSON3(json3Data, language);
